...
const addFashionProduct = asyncHandler(async (req, res) => {
    ...
    if (!name || !brandName || !discription || !size || !fashionCategory || !status || !type || !price || !quantity) {
      res.status(400)
    ...
    const fashionProduct = await FashionProduct.create({
        name ,brandName ,discription ,size ,fashionCategory ,status ,type ,price ,quantity
    })
